...
static struct buffer *buffer_alloc(unsigned long size)
{
	...
	printk(KERN_INFO "Abin, buffer_alloc function called");
	...
	if (unlikely(!buf->data))
		goto out_free;
	/*wait_queue_head_t must be explicitly initialized with 
	either init_waitqueue_head at runtime or
	 DECLARE_WAIT_QUEUE_HEAD at compile time*/
	init_waitqueue_head(&buf->read_queue);
	/* Since the memory for such a mutex must be allocated 
	dynamically, the mutex needs to be explicitly initialized
	 using mutex_init() */
	mutex_init(&buf->lock);
...
static void buffer_free(struct buffer *buffer)
{
        ...
        printk(KERN_INFO "Abin, buffer_free function called");
	kfree(buffer->data);
	kfree(buffer);
}
...
static inline char *reverse_word(char *start, char *end)
{
	char *orig_start = start, tmp;
        ...
        printk(KERN_INFO "Abin, reverse_word function called");
...
{
        printk(KERN_INFO "Abin, reverse_open function called");
	struct buffer *buf;
	int err = 0;
...
{
        printk(KERN_INFO "Abin, reverse_read function called");
			     ...
			     size_t size, loff_t * off)
{
	...
	ssize_t result;
        ...
        printk(KERN_INFO "Abin, reverse_write function called");
	if (size > buffer_size) {
		result = -EFBIG;
		goto out;
	}
...
{
        printk(KERN_INFO "Abin, reverse_close function called");
	struct buffer *buf = file->private_data;
